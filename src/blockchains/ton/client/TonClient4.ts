...
export class TonClient4 {
	 ...
	 */
	async getLastBlock() {
		const res = await axios.get(this.#endpoint + '/block/latest', {
			adapter: this.#adapter,
			timeout: this.#timeout,
		});
		const lastBlock = lastBlockCodec.safeParse(res.data) as any;
	...
	async getBlock(seqno: number) {
		const res = await axios.get(this.#endpoint + '/block/' + seqno, {
			adapter: this.#adapter,
			timeout: this.#timeout,
		});
		const block = blockCodec.safeParse(res.data);
		if (!block.success) {
	...
	async getBlockByUtime(ts: number) {
		const res = await axios.get(this.#endpoint + '/block/utime/' + ts, {
			adapter: this.#adapter,
			timeout: this.#timeout,
		});
		const block = blockCodec.safeParse(res.data);
	...
	async getAccount(seqno: number, address: Address) {
		const res = await axios.get(this.#endpoint + '/block/' + seqno + '/' + address.toString({ urlSafe: true }), {
			adapter: this.#adapter,
			timeout: this.#timeout,
		});
		const account = accountCodec.safeParse(res.data);
		if (!account.success) {
			throw Error('Mailformed response');
		}
		return account.data;
	...
	async getAccountLite(seqno: number, address: Address) {
		const res = await axios.get(
			this.#endpoint + '/block/' + seqno + '/' + address.toString({ urlSafe: true }) + '/lite',
			{ adapter: this.#adapter, timeout: this.#timeout },
		);
		const account = accountLiteCodec.safeParse(res.data);
		if (!account.success) {
			throw Error('Mailformed response');
		}
		return account.data;
	...
	async isContractDeployed(seqno: number, address: Address) {
		const account = await this.getAccountLite(seqno, address);
	...
	async isAccountChanged(seqno: number, address: Address, lt: bigint) {
		...
		const changed = changedCodec.safeParse(res.data);
		if (!changed.success) {
			throw Error('Mailformed response');
		}
		return changed.data;
	...
	async getAccountTransactions(address: Address, lt: bigint, hash: Buffer) {
		...
		const transactions = transactionsCodec.safeParse(res.data);
		...
		const cells = Cell.fromBoc(Buffer.from(data.boc, 'base64'));
		for (let i = 0; i < data.blocks.length; i++) {
			tx.push({
				block: data.blocks[i] as any,
	...
	async getAccountTransactionsParsed(address: Address, lt: bigint, hash: Buffer, count: number = 20) {
		...
		const parsedTransactionsRes = parsedTransactionsCodec.safeParse(res.data);
	...
	async getConfig(seqno: number, ids?: number[]) {
		...
		const res = await axios.get(this.#endpoint + '/block/' + seqno + '/config' + tail, {
			adapter: this.#adapter,
			timeout: this.#timeout,
		});
		const config = configCodec.safeParse(res.data);
		if (!config.success) {
			throw Error('Mailformed response');
		}
		return config.data;
	...
	async runMethod(seqno: number, address: Address, name: string, args?: TupleItem[]) {
		...
		const runMethod = runMethodCodec.safeParse(res.data);
		if (!runMethod.success) {
			throw Error('Mailformed response');
		}
		const resultTuple = runMethod.data.resultRaw
			? parseTuple(Cell.fromBoc(Buffer.from(runMethod.data.resultRaw, 'base64'))[0])
			: [];
		return {
	...
	async sendMessage(message: Buffer) {
		...
		const send = sendCodec.safeParse(res.data);
		if (!send.success) {
			throw Error('Mailformed response');
		}
...
function createProvider(
	...
	return {
		async getState(): Promise<ContractState> {
			...
			let sq = block;
			if (sq === null) {
				const res = await client.getLastBlock();
				sq = res.last.seqno;
			}
			...
			// Convert state
			const last = state.account.last
				? { lt: BigInt(state.account.last.lt), hash: Buffer.from(state.account.last.hash, 'base64') }
				: null;
			let storage:
				| {
						type: 'uninit';
		...
		async get(name, args) {
			...
			const method = await client.runMethod(sq!, address, name, args);
